...
import com.google.android.exo.extractor.mp4.PsshAtomUtil;
import com.google.android.exo.drm.DefaultDrmSession.ProvisioningManager;
import com.google.android.exo.drm.DrmSession.DrmSessionException;
...
public class DefaultDrmSessionManager<T extends ExoMediaCrypto> implements DrmSessionManager<T>,
  ...
  public boolean canAcquireSession(@NonNull DrmInitData drmInitData) {
    ...
    DrmInitData.SchemeData schemeData = getSchemeData(drmInitData, uuid, true);
    if (schemeData == null) {
  ...
  public DrmSession<T> acquireSession(Looper playbackLooper, DrmInitData drmInitData) {
    ...
    if (offlineLicenseKeySetId == null) {
      DrmInitData.SchemeData data = getSchemeData(drmInitData, uuid, false);
      if (data == null) {
  ...
  /**
   * Extracts {@link DrmInitData.SchemeData} suitable for the given DRM scheme {@link UUID}.
   *
   * @param drmInitData The {@link DrmInitData} from which to extract the {@link DrmInitData.SchemeData}.
   * @param uuid The UUID.
   * @param allowMissingData Whether a {@link DrmInitData.SchemeData} with null {@link DrmInitData.SchemeData#data} may be
   *     returned.
   * @return The extracted {@link DrmInitData.SchemeData}, or null if no suitable data is present.
   */
  private static DrmInitData.SchemeData getSchemeData(DrmInitData drmInitData, UUID uuid,
    ...
    if (C.WIDEVINE_UUID.equals(uuid)) {
      for (int i = 0; i < matchingSchemeDatas.size(); i++) {
        DrmInitData.SchemeData matchingSchemeData = matchingSchemeDatas.get(i);
  ...
  private static byte[] getSchemeInitData(DrmInitData.SchemeData data, UUID uuid) {
  ...
  private static String getSchemeMimeType(DrmInitData.SchemeData data, UUID uuid) {
