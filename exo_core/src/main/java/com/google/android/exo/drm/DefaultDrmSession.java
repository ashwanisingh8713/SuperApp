...
/* package */ class DefaultDrmSession<T extends ExoMediaCrypto> implements DrmSession<T> {
  ...
  private void postKeyRequest(int type, boolean allowRetry) {
    ...
    try {
      ExoMediaDrm.KeyRequest request = mediaDrm.getKeyRequest(scope, initData, mimeType, type,
          optionalKeyRequestParameters);
      if (C.CLEARKEY_UUID.equals(uuid)) {
        request = new ExoMediaDrm.DefaultKeyRequest(ClearKeyUtil.adjustRequestData(request.getData()),
  ...
  private class PostRequestHandler extends Handler {
    ...
    public void handleMessage(Message msg) {
      ...
      try {
        switch (msg.what) {
          case MSG_PROVISION:
            response = callback.executeProvisionRequest(uuid, (ExoMediaDrm.ProvisionRequest) msg.obj);
            break;
          case MSG_KEYS:
