package com.ns.view.roundedimageview;
...
public class RoundedImageView extends BaseImgView {
    ...
    public RoundedImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        final int placeHolder = a.getInt(R.styleable.RoundedImageView_riv_place_holder, -1);
        if (placeHolder != -1) {
            updateIcon(placeHolder);
        }
    ...
    @Override
    public void updateIcon(int placeHolderType) {
        ...
        if(tableConfiguration != null && THPConstants.IS_USE_SEVER_THEME) {
            if(isUserThemeDay) {
                loadIconsFromServer(placeHolderType, tableConfiguration.getOtherIconsDownloadUrls().getLight().getPlaceHolder(), FileUtils.destinationFolder(getContext(), FileUtils.PLACE_HOLDER_LIGHT).getPath());
            } else {
                loadIconsFromServer(placeHolderType, tableConfiguration.getOtherIconsDownloadUrls().getDark().getPlaceHolder(), FileUtils.destinationFolder(getContext(), FileUtils.PLACE_HOLDER_DARK).getPath());
            }
        }
    ...
    private void loadIconsFromServer(int placeHolderType, PlaceHolder listingIconUrl, String destinationFolderPath) {

        String iconUrl = "";

        // 0 = app:riv_place_holder="banner"
        if(placeHolderType == 0) {
            iconUrl = listingIconUrl.getBanner();
        }
        // 1 = app:riv_place_holder="thumb"
        else if(placeHolderType == 1) {
            iconUrl = listingIconUrl.getThumb();
        }
        // 2 = app:riv_place_holder="widget"
        else if(placeHolderType == 2) {
            iconUrl = listingIconUrl.getWidget();
        }
        // 3 = app:riv_place_holder="ads"
        else if(placeHolderType == 3) {
            iconUrl = listingIconUrl.getAds();
        }
